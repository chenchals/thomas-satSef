...
for k=1:length(x)
    if ~isempty(lx) & ~isempty(ly) % both errors are specified
        ...
        hold on;
        l2=line([0 0],[0 0]);%line([lx(k) lx(k)],[y(k)-0.1*mean(erry) y(k)+0.1*mean(erry)]);
        l3=line([0 0],[0 0]);%line([ux(k) ux(k)],[y(k)-0.1*mean(erry) y(k)+0.1*mean(erry)]);
        l4=line([x(k) x(k)],[ly(k) uy(k)]);
        l5=line([0 0],[0 0]);%line([x(k)-0.1*mean(errx) x(k)+0.1*mean(errx)],[ly(k) ly(k)]);
        l6=line([0 0],[0 0]);%line([x(k)-0.1*mean(errx) x(k)+0.1*mean(errx)],[uy(k) uy(k)]);
